  ...
  class Decider
    ...
    def memes(opts={})
      ...
      begin
        ...
        TwitterClient.send_private("Run #{run.id} / Memes #{umras.size} / S #{(((strongest_meme.strength_z_score)*100).floor.to_f)/100.0} / T #{((((run.ended_at - run.started_at) / 60) * 100).floor.to_f)/100} min")
      rescue
