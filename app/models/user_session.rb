  ...
  def save(&block)
    if authenticating_with_openid?
      ...
      controller.send(:authenticate_with_open_id, openid_identifier) do |result, openid_identifier|
        ...
        record = nil
        if controller.cookies[:u]
          record = User.find_using_perishable_token(controller.cookies[:u], 1.week) || (raise Exception)
          record.active = true
          record.openid_identifier = controller.params["openid.claimed_id"]
          if record.save
            controller.cookies[:u] = { :value => "", :expires => (Time.now-999) }
          end
        else
